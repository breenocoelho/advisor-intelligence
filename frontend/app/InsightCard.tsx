...
export default function InsightCard({
  ...
  const [description, setDescription] = useState(insight.explanation ?? "");
  ...
  async function updateStatus(newStatus: string) {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/insights/${insight.id}?new_status=${newStatus}`,
        {
          method: "PATCH",
  ...
  async function submitTask() {
    ...
    try {
      const token = await getToken();
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/insights/${insight.id}/tasks`, {
        method: "POST",
        headers: {
      ...
      >
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline justify-between gap-3">
            <p className="truncate font-medium">{insight.client_name ?? "Cliente"}</p>
          ...
          <p className="mt-1 text-sm leading-relaxed text-[#14181F]/70">{insight.explanation}</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#14181F]/40 px-4">
          <div className="w-full max-w-sm rounded-lg bg-white p-5 shadow-lg">
            ...
            <p className="mt-1 text-sm text-[#14181F]/50">
              {insight.client_name ?? "Cliente"} — {typeLabel}
            </p>
